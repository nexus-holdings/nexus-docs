...
| **Registered via** | `POST /api/companies/{companyId}/routines` on startup — `bootstrap_routines.py` reads the catalog and registers each, so `GET /api/companies/<id>/routines` returns the registered set |
